...
/*	//设置初始化内部需求的状态。
	...
	for (int index = 0; index < iElevatorFloorsCount; index++)
	{
		SendMessage(this->hElevatorInnerBtnWnd, LB_INSERTSTRING, (WPARAM)-1, (LPARAM)szFloorButtonHint[index]);
		//设置每个按钮的高度。
		SendMessage(this->hElevatorInnerBtnWnd, LB_SETITEMHEIGHT, (WPARAM)index, (LPARAM)iButtonItemHeight);
	}
	//初始状态下电梯静止停靠在第一层。
